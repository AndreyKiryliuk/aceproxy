...
import string, os, urllib, cookielib, time
...
fcookies = os.path.join('/', r'cookies.txt')
...
def cleartext(str):
    str = str.replace("http://s.rutor.info/", '/s/')
...
def ShowRoot():
    ...
    for cat in CATEGORIES:
        Title = "[%s]" % cat['title']
        itemdict = {'name': Title,
                    'url': 'category/%s/' % cat['id'],
                    'description_title': Title,
                    'description': '',
                    }
        playlist.addItem(itemdict)
    # return playlist.exportxml
    return ''
...
db_name = os.path.join('/', "move_info.db")
...
def get_minfo(ntor):
        KPoisk = 'false'
        try: dict = eval(get_inf_db(ntor)[0][0])
        except:
            import kpi
            hp = GET(httpSiteUrl + '/torrent/' + ntor, httpSiteUrl, None)
            id = kp_id(hp)
...
LstDir = os.path.abspath('/')
def OpenCat(url, name, dict):
    nnn = url.rfind("/") + 1
    ntor = url[nnn:] + ".torrent"
    rtpath = ru(os.path.join(LstDir, ntor))
    urllib.urlretrieve(url, rtpath)
...
def SearchN(category, sort, text, filtr, page='0'):
    if text == "   ": text = "0"
    if text == "    ": text = "0"
    # url='0'
#     HideScr = __settings__.getSetting("Hide Scr")
